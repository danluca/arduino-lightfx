...
namespace nina {
    WiFiClass::WiFiClass() : _timeout(50000), _feed_watchdog_func(nullptr) {
    }
    ...
    uint8_t WiFiClass::beginAP(const char *ssid) const {
    ...
    uint8_t WiFiClass::beginEnterprise(const char *ssid, const char *username, const char *password,
                                       const char *identity, const char *ca) const {
        uint8_t status = WL_IDLE_STATUS;

        // set passphrase
        if (WiFiDrv::wifiSetEnterprise(0 /*PEAP/MSCHAPv2*/, ssid, strlen(ssid), username, strlen(username), password,
                                       strlen(password), identity, strlen(identity), ca,
                                       strlen(ca) + 1) != WL_FAILURE) {
            for (unsigned long start = millis(); (millis() - start) < _timeout;) {
                delay(WL_DELAY_START_CONNECTION);
                status = WiFiDrv::getConnectionStatus();
                if ((status != WL_IDLE_STATUS) && (status != WL_NO_SSID_AVAIL) && (status != WL_SCAN_COMPLETED)) {
    ...
    uint8_t *WiFiClass::macAddress(uint8_t *mac) {
        const uint8_t *_mac = WiFiDrv::getMacAddress();
    ...
    float WiFiClass::getTemperature() {
