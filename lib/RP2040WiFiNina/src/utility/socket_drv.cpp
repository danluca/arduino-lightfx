...
namespace {
    ...
    struct LastError {
        ...
        void reset() { this->present = false; }
        LastError& operator=(const uint8_t val) {
            this->present = true;
            this->value = val;
            return *this;
        }
        operator bool() const { return this->present; }
    };
}
...
uint8_t SocketDrv::socket(const uint8_t type, const uint8_t proto) {
...
bool SocketDrv::close(const uint8_t s) {

    g_lastError.reset();
    if (!SpiDrv::initialized)
		SpiDrv::begin();

    constexpr uint8_t cmd = SOCKET_CLOSE_CMD;
    {
        SelectSlave sel;
...
uint8_t SocketDrv::lastError() {
    ...
    constexpr uint8_t cmd = SOCKET_ERRNO_CMD;
    {
        SelectSlave sel;
...
bool SocketDrv::bind(const uint8_t s, const uint16_t port) {
...
bool SocketDrv::listen(const uint8_t s, const uint8_t backlog) {
    g_lastError.reset();
    if (!SpiDrv::initialized)
		SpiDrv::begin();
...
uint8_t SocketDrv::accept(const uint8_t s, IPAddress & remoteIpAddress, uint16_t & remotePort) {
    g_lastError.reset();
    if (!SpiDrv::initialized)
		SpiDrv::begin();

    constexpr uint8_t cmd = SOCKET_ACCEPT_CMD;
    {
        SelectSlave sel;
    ...
    }
    {
        ...
        if (resultSocket != 255) {
            g_lastError = 0;
            remoteIpAddress = IPAddress(remoteIpBuf); //buf is in network order
...
bool SocketDrv::connect(const uint8_t s, const IPAddress & ipAddress, const uint16_t port) {
    ...
    constexpr uint8_t cmd = SOCKET_CONNECT_CMD;
...
int32_t SocketDrv::send(const uint8_t s, const void * buf, uint16_t size) {
    g_lastError.reset();
    if (!SpiDrv::initialized)
		SpiDrv::begin();
    ...
    constexpr uint8_t cmd = SOCKET_SEND_CMD;
