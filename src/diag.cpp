...
void diagSetup() {
    ...
    Log.info(F("Diagnostic thread [%s] - priority %lu - has been setup id %lu. Events are dispatching."), diagTask->getName(),
...
void readCalibrationInfo() {
    ...
    if (const size_t calibSize = readTextFile(calibFileName, json); calibSize > 0) {
        JsonDocument doc;
