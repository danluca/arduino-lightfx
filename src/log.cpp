...
void printLogLevel(Print *_logOutput, int logLevel) {
    ...
    switch (logLevel) {
        default:
        case 0: _logOutput->print("SLN ");
            break;
        case 1: _logOutput->print("FTL ");
            break;
        case 2: _logOutput->print("ERR ");
            break;
        case 3: _logOutput->print("WRN ");
            break;
        case 4: _logOutput->print("INF ");
            break;
        case 5: _logOutput->print("TRC ");
            break;
        case 6: _logOutput->print("VRB ");
            break;
    }
#endif
}
...
void printRGB(Print *const out, const CRGB rgb) {
    uint32_t numClr = rgb.as_uint32_t() & 0xFFFFFF;
    //conversion to uint32 uses 0xFF for the alpha channel - we're not interested in the alpha channel
    out->print("0x");
    int8_t padding = 5 - Logging::countSignificantNibbles(numClr);
    //3 bytes, 6 digits 0 padded, accounting for at least one digit being printed
    while (padding-- > 0)
        out->print('0');
